...
# React 19.2 Development Guide

## Core Directives
- **React Compiler Enabled**: Assume the React Compiler handles memoization. Do NOT use `useMemo` or `useCallback` unless specifically requested for stable object identities (e.g., for external libraries).
- **Ref Handling**: NEVER use `forwardRef`. Treat `ref` as a standard prop in function components.
  - Example: `function MyInput({ ref, ...props }) { return <input ref={ref} {...props} />; }`
- **Data Fetching**: Prefer the `use()` hook over `useEffect` for reading promises. Integrate with <Suspense> and <ErrorBoundary>.

## Hook-Specific Rules
### 1. The `use()` Hook
- **Resource Reading**: Use `use(promise)` to read async data directly in render.
- **Conditional Use**: Unlike other hooks, `use()` CAN be called inside loops or `if` statements.
- **Cache Warning**: Ensure promises passed to `use()` are cached (e.g., via `cache()` in RSC or a stable reference) to prevent infinite re-render loops.

### 2. Form Actions & Transitions
- **`useActionState`**: Use this for form-related state (formerly `useFormState`). It provides `[state, formAction, isPending]`.
- **`useFormStatus`**: Use this in child components of a `<form>` to access `pending` status without manual prop drilling.
- **`useOptimistic`**: Use for immediate UI feedback during async transitions.

## Anti-Pattern: The useEffect Trap
Avoid "Effect Soup" by following these strict constraints:
1. **No Data Fetching**: Stop using `useEffect` for API calls. Use `use()` or a library like TanStack Query.
2. **Race Conditions**: Avoid manual `ignore` flags in effects. `use()` handles the lifecycle automatically with Suspense.
3. **Event Logic**: If an effect needs to read props/state without re-triggering, use the new `useEffectEvent` hook.
4. **State Transitions**: If state change depends on other state, use **derived state** (const) or `key` to reset a component's state instead of an effect.

## Testing (React 19.2+)
- **Async Verification**: Use `act()` for all async updates and transitions.
- **Transition Testing**: Verify `isPending` states explicitly in UI tests.
