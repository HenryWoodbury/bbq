...
Global data shared across all leagues. The canonical player list is sourced from the [Smart Fantasy Baseball Player ID Map](https://www.smartfantasybaseball.com/PLAYERIDMAPCSV) — ~750 fantasy-relevant players updated infrequently. Admins trigger a remote sync via the Admin UI. Admins and Commissioners can also add players to the list.

The player universe is supplemented by a format-specific table (e.g. Ottoneu) that provides the positions for each player in the designated format.
