...
function resolveGlob(dir: string, prefix: string): string {
  ...
  if (files.length === 0) throw new Error(`No file matching ${prefix}*.csv in ${dir}`)
  return join(dir, files[0])
}

function resolvePaths(args: string[]): { idmapPath: string; universePath: string } {
  if (args.length >= 2) {
...
async function seedPlayers(csvPath: string): Promise<{ inserted: number; updated: number; deleted: number }> {
  ...
  if (idx.sfbbId === -1 || idx.playerName === -1) {
    throw new Error(`Missing required columns IDPLAYER/PLAYERNAME. Found: ${headers.slice(0, 15).join(", ")}`)
  }

  type Row = {
    sfbbId: string; playerName: string; fgSpecialChar: string | null
    firstName: string | null; lastName: string | null; positions: string[]
    team: string | null; mlbLevel: string | null; active: boolean
    birthday: Date | null; bats: string | null; throws: string | null
    mlbamId: number | null; fangraphsId: string | null; cbsId: number | null
    espnId: number | null; yahooId: number | null; fantraxId: string | null
    retroId: string | null; nfbcId: number | null; bRefId: string | null
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const birthday = rawBirthday
      ? (() => { const d = new Date(rawBirthday); return Number.isNaN(d.getTime()) ? null : d })()
      : null
  ...
  if (rows.length === 0) throw new Error("No valid rows parsed from PLAYERIDMAP CSV")
  process.stdout.write(`  Parsed ${rows.length.toLocaleString()} rows\n`)

  const existingIds = new Set(
    (await prisma.player.findMany({ where: { sfbbId: { in: rows.map((r) => r.sfbbId) } }, select: { sfbbId: true } }))
      .map((p) => p.sfbbId),
  )
    ...
    await prisma.$transaction(
      batch.map((row) =>
        prisma.player.upsert({
          ...
          create: {
            sfbbId: row.sfbbId, playerName: row.playerName,
            fgSpecialChar: row.fgSpecialChar, firstName: row.firstName,
            lastName: row.lastName, positions: row.positions,
            team: row.team, mlbLevel: row.mlbLevel, active: row.active,
            birthday: row.birthday, bats: row.bats, throws: row.throws,
            mlbamId: row.mlbamId, fangraphsId: row.fangraphsId,
            cbsId: row.cbsId, espnId: row.espnId, yahooId: row.yahooId,
            fantraxId: row.fantraxId, retroId: row.retroId,
            nfbcId: row.nfbcId, bRefId: row.bRefId, ottoneuId: row.ottoneuId,
          },
          update: {
            playerName: row.playerName, fgSpecialChar: row.fgSpecialChar,
            firstName: row.firstName, lastName: row.lastName,
            positions: row.positions, team: row.team, mlbLevel: row.mlbLevel,
            active: row.active, birthday: row.birthday, bats: row.bats,
            throws: row.throws, mlbamId: row.mlbamId, fangraphsId: row.fangraphsId,
            cbsId: row.cbsId, espnId: row.espnId, yahooId: row.yahooId,
            fantraxId: row.fantraxId, retroId: row.retroId,
            nfbcId: row.nfbcId, bRefId: row.bRefId, ottoneuId: row.ottoneuId,
            deletedAt: null,
...
async function seedUniverse(csvPath: string): Promise<{ inserted: number; updated: number; deleted: number }> {
  ...
  const headers = parseCSVLine(lines[0])
  const idx = Object.fromEntries(
    Object.entries(UNIVERSE_COL).map(([key, col]) => [key, headers.indexOf(col)]),
  ) as Record<keyof typeof UNIVERSE_COL, number>

  if (idx.ottoneuId === -1 || idx.playerName === -1) {
    throw new Error(`Missing required columns. Found: ${headers.slice(0, 10).join(", ")}`)
  }

  type Row = {
    ottoneuId: number; playerName: string; fangraphsId: string | null
    mlbamId: number | null; birthday: Date | null; positions: string[]
  }
  ...
  if (rows.length === 0) throw new Error("No valid rows parsed from Player Universe CSV")
  process.stdout.write(`  Parsed ${rows.length.toLocaleString()} rows\n`)
  ...
  const playerIdByOttoneuId = new Map(matchedPlayers.map((p) => [p.ottoneuId, p.id]))
  ...
  const { count: deleted } = await prisma.playerUniverse.updateMany({
    where: { format: "ottoneu", ottoneuId: { notIn: uploadedOttoneuIds }, deletedAt: null },
    data: { deletedAt: new Date() },
  })
